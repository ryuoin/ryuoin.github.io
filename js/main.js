...
/**
 * 주식 시장 개장 여부 확인 (주말 및 2026년 법정 공휴일/휴장일 판별)
 */
function isMarketClosed() {
    ...
    // 2. 2026년 대한민국 주식 시장(KRX) 주요 휴장일 리스트 (YYYY-MM-DD)
    const krxHolidays2026 = [
        "2026-01-01", // 신정
        "2026-02-16", "2026-02-17", "2026-02-18", "2026-02-19", // 설날 연휴 및 대체공휴일
        "2026-03-02", // 삼일절 대체공휴일 (3/1이 일요일)
        "2026-05-05", // 어린이날
        "2026-05-25", // 부처님오신날 대체공휴일 (5/24이 일요일)
        "2026-06-06", // 현충일
        "2026-08-17", // 광복절 대체공휴일 (8/15이 토요일)
    ...
    return krxHolidays2026.includes(dateStr);
}
...
function initLotto() {
    ...
    if (spinner) {
        ...
        for (let i = 1; i <= 45; i++) {
            ...
            const duration = (2.0 + Math.random() * 2.0).toFixed(2); 
...
function showStockResult() {
    ...
    if (verdictArea) {
        ...
        verdictArea.style.removeProperty('display');
